...
@Composable
fun HomeScreen(viewModel: HomeViewModel, onDogList: (choice: Int) -> Unit) {
    LaunchedEffect(Unit) {
...
@Composable
private fun HomeScreenContent(images: List<DogImage>, onClickButton: (Int) -> Unit) {
    if (images.isNotEmpty()) {
        Column(modifier = Modifier.padding(vertical = 25.dp)) {
            ...
            Row(
                ...
                Button(onClick = { onClickButton(ChoicesEnum.BY_CATEGORY.choice) }) {
                    Text(
                        text = "By Category",
                ...
                Button(onClick = { onClickButton(ChoicesEnum.MY_FAVOURITES.choice) }) {
                    Text(
                        text = "My Favourites",
                        style = MaterialTheme.typography.h5,
                    )
            ...
            Button(
                ...
                onClick = {
                    onClickButton(ChoicesEnum.SEE_ALL.choice)
                }
            ) {
